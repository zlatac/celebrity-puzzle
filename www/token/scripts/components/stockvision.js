...
let projectStockVision = function (codeInput, manualEntryPrice, manualExitPrice, isCrypto = false, entryPercentage, exitPercentage, tradingInterval = '1hour', precisionInterval = '5min') {
    ...
    const mutationObserverCallback = (mutationArray, observerInstance, inspectorTrigger) => {
        try {
            ...
            let entryPrice = idaStockVision.settings[code].manualEntryPriceThreshold
            let exitPrice = idaStockVision.settings[code].manualExitPriceThreshold
            const entryPercentageThreshold = idaStockVision.settings[code].entryPercentageThreshold
            const exitPercentageThreshold = idaStockVision.settings[code].exitPercentageThreshold
            let analysis, anchor
            /** @type {number | string} */
            let anchorPrice = ''
